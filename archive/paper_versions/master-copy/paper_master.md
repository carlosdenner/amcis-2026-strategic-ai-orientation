...
Artificial intelligence has moved from boardroom aspiration to organizational imperative. McKinsey's latest global survey finds that 88% of organizations report using AI in at least one business function, yet the majority remain in experimentation or piloting stages and only about one-third have begun scaling [@mckinseyai2025]. The gap between AI ambition and AI realization is widening, not narrowing. @bcg2025 report that over 50% of firms cite legacy IT architectures as the primary barrier to AI scaling, while 74% of AI-leading companies — but far fewer laggards — continuously monitor responsible AI (RAI) framework compliance. @deloitte2026 argue that agentic AI value comes not from model selection alone but from redesigning operations and building agent-compatible architectures.
...
We frame this tension through the lens of the CIO's evolving role. Upper echelons theory [@hambrick1984] establishes that executive cognition and attention shape strategic choices. Recent IS research extends this to AI contexts, showing that CIO centrality and board AI awareness significantly influence a firm's AI orientation [@misq2021]. Yet orientation alone does not produce outcomes. Drawing on the dynamic managerial capabilities framework [@adner2003; @hossain2025], we argue that organizations require two distinct capability bundles to convert orientation into value:

1. **Trust readiness** — the governance capability bundle encompassing risk management, compliance, adversarial threat modeling, incident response, and stakeholder accountability. This is grounded in normative frameworks including NIST AI RMF 1.0 [@nist2023], the EU AI Act [@euaiact2024], OWASP Top 10 for LLM Applications [@owasp2025], and the MITRE ATLAS adversarial threat taxonomy.
...
Upper echelons theory posits that organizational outcomes are partially predicted by the characteristics, experiences, and attention patterns of top management team members [@hambrick1984]. The CIO's role has evolved from a technology steward to a strategic partner [@chawla2023], with digital transformation requiring CIOs to operate simultaneously across technical, business, and institutional domains. In AI contexts, this multidimensionality becomes critical: AI orientation — a firm's strategic commitment to AI as a source of competitive advantage — is shaped by CIO centrality (board access, reporting structure, agenda ownership) and board AI awareness [@misq2021].

@mckinsey2026 frame the contemporary CIO mandate around three imperatives: strategy, speed, and scaled intelligence. @bcg2025 provide empirical backing: at 86% of AI-leading companies, IT leads or co-leads generative AI initiatives, compared to only 54% at AI-stagnating firms. @gartner2026 identifies agility and risk mastery as the defining items on the CIO agenda, both requiring strategic-level AI orientation. These converging practitioner findings support the theoretical claim that CIO centrality is a necessary antecedent of organizational AI orientation.

### Dynamic Managerial Capabilities and Governance-as-Capability

While upper echelons theory explains *why* CIO attention matters for AI orientation, it does not explain *how* orientation converts into realized value. For this, we draw on the dynamic managerial capabilities (DMC) framework [@adner2003], which identifies managerial human capital, managerial social capital, and managerial cognition as the micro-foundations of capability building. @hossain2025 extend DMC to digital leadership contexts, arguing that digital-era executives must develop capabilities that span both technical and institutional domains.

We complement DMC with a governance-as-capability perspective. Rather than treating governance as a static compliance constraint, we conceptualize it as a dynamic organizational capability — a bundle of routines, skills, and processes that enables the firm to sense governance requirements, seize governance opportunities (e.g., building trust as competitive advantage), and reconfigure governance structures as regulatory and technological landscapes evolve [@teece2007]. This framing aligns with ISACA's COBIT for AI [-@isaca2024], which defines governance not as overhead but as a set of objectives that organizations must actively build competency in.
...
- **Regulatory compliance** (EU AI Act Art. 9–15 [@euaiact2024], ISO/IEC 42001 [@iso42001], ISO/IEC 23894 [@iso23894])
...
@bcg2025 find that over 50% of firms identify legacy IT architecture as the primary barrier to AI scaling, underscoring the strategic importance of integration readiness as a capability rather than a fixed asset.

### Institutional Decoupling and the Risk of Governance Theater

A persistent finding in institutional theory is that organizations adopt formal structures to signal conformity with institutional expectations while decoupling those structures from day-to-day operations [@meyer1977]. @bromley2012 refine this insight by distinguishing *policy-practice decoupling* — where adopted policies are not implemented — from *means-ends decoupling* — where implemented practices fail to achieve their intended outcomes. Both forms are relevant to AI governance: an organization may adopt an internal review board without conducting substantive assessments (policy-practice), or it may conduct impact assessments that do not trigger downstream mitigation (means-ends). Recent work confirms that such decoupling dynamics extend to AI ethics initiatives specifically: @ali2023 document how organizational structures create barriers between stated AI ethics commitments and operational practice, producing what @seele2022 term "machinewashing" — the AI analog of greenwashing. We adopt the term *governance theater* to describe this decoupling pattern in the AI governance context, and our empirical analysis tests whether the EO 13960 inventory exhibits the signatures of both forms.

### Complementarity of Trust and Integration Readiness

We theorize that trust readiness and integration readiness are strategic complements [@milgrom1990]: the marginal value of each capability bundle increases when the other is also present. This complementarity arises because:
...
We follow a *convergent triangulation* design [@creswell2018] adapted for secondary data. Three data streams are independently collected, profiled, and coded, then merged through a cross-taxonomy mapping that bridges their distinct classification schemes. The triangulation logic connects:
...
We anchor the bridging table to McKinsey's empirically derived "constraints to AI scaling" [@mckinsey2026, Exhibit 5], which reports eight barriers cited by CIOs in a global survey:
...
**Mapping validation.** Because a single researcher produced the mapping (no second coder for inter-rater reliability), we validate robustness through Monte Carlo sensitivity analysis (`scripts/11_mapping_sensitivity.py`). Across 1,000 iterations at each of three perturbation rates (10%, 15%, 20% of links randomly reassigned to different constraints), the mapping structure is highly stable: the top constraint (C3: security/reliability) is preserved in 100% of iterations at all perturbation rates; the top-2 set {C3, C4} is preserved in 99.9% of iterations even at 20% perturbation; the mean Spearman rank correlation between perturbed and baseline constraint distributions is ρ=0.95 (SD=0.05) at 20% perturbation; and all three source taxonomies retain coverage of both C3 and C4 in 93%+ of iterations. Critically, the paper’s core empirical findings — the governance drop-off (61% → 5–9%), procurement opacity, and IR > TR as deployment predictor — are computed directly from EO 13960 variables and are not mediated by the cross-taxonomy map. The map underpins the triangulation logic (§4.5), and the sensitivity analysis confirms that this triangulation structure is robust to substantial perturbation of mapping decisions.
...
**False discovery mitigation.** Because 400 experiments raise legitimate concerns about multiple comparisons and researcher degrees of freedom, we adopt four safeguards. First, we treat *single-experiment support as suggestive only*; we emphasize findings only when supported by *convergent evidence across multiple independent experiments* testing the same substantive claim through different operationalizations. Second, the AutoDiscovery engine generates hypotheses programmatically from the data schema rather than through post-hoc researcher selection, reducing garden-of-forking-paths bias. Third, the high contradiction rate (67.7%) demonstrates that the engine does not selectively confirm hypotheses — the majority of tested expectations are falsified, indicating genuine discriminative power rather than confirmation bias. Fourth, the paper’s core quantitative findings (governance drop-off, IR > TR, procurement opacity) are independently validated through standard statistical methods (logistic regression, chi-square tests, Spearman correlations) reported with conventional p-values and effect sizes in §4.1–§4.8, providing a confirmatory cross-check on the exploratory discovery results.
...
| Commercial opacity barrier | 14 | 106, 131, 174, 202, 210, 237, 245, 247, 284, 291, 293, 299 | §4.6 |
| Sector-specific incident fingerprints (harm + failure) | 7 | 158, 168, 170, 173, 187, 242, 252 | χ²=12.97, p=0.0003 |
...
| Transparency-without-agency (public-facing) | 5 | 134, 160, 180, 159, 200 | §4.7 |
| EO mandate ineffective on deep safeguards | 3 | 182, 241, 268 | §4.1 |
| IR > TR as deployment predictor | Regression | — | OR=1.40, p<0.001 (§4.8) |
...
python scripts/13_aiid_coverage_robustness.py  # → aiid_coverage_robustness.csv
python scripts/14_atlas_threat_characterization.py  # → ATLAS threat analysis + Table 6
```
...
Taken together, Section 4.1 establishes the paper's core empirical anchor: the EO inventory records internal review processes without commensurate evidence of "capacity to govern." In institutional-theory terms, this is a textbook case of policy-practice decoupling [@meyer1977; @bromley2012]: organizations adopt the formal structures of governance (review boards, authorization processes) to satisfy institutional expectations, while the operational routines that would make those structures consequential — impact assessment, independent evaluation, bias mitigation — remain largely absent.
...
Operational deployment rates differ sharply by IR level: 33% for low-IR systems (0–2, n=772), 57% for mid-IR (3–4, n=335), and 73% for high-IR (5–7, n=650). The near-doubling from low to high IR is consistent with the logistic regression finding (§4.8) that IR is the dominant predictor of deployment.
...
This finding directly supports the complementarity thesis at a descriptive level, even though the regression interaction (§4.8) is not yet positive: the preconditions for complementarity (co-occurrence of IR and deep TR) exist in the high-IR tail of the distribution, but they are too rare at current maturity levels to produce a detectable interaction in the full sample. The Session 1 structural finding that trust and integration failures are intertwined in incident contexts (mixed clusters; EXP_055) provides convergent support: when adversarial cases and incidents are examined, the failure modes that produce real harm typically span both governance and architectural domains simultaneously, implying that the two readiness bundles are functionally coupled even when organizationally siloed.

### 4.3 Adversarial Threat Structure: ATLAS Attack Complexity and Mitigation Demand

**Finding:** Adversarial AI attacks documented in MITRE ATLAS are multi-stage, resource-intensive operations whose most-needed mitigations map directly onto the governance safeguards least prevalent in federal practice.

Across the 52 ATLAS case studies, resolved tactic frequency analysis (`scripts/14_atlas_threat_characterization.py`) reveals that Resource Development appears in 77 cases (148% — most cases use it multiple times), followed by Impact (90%), Initial Access (79%), Execution (71%), Defense Evasion (65%), and AI Attack Staging (63%). The median case traverses 6 distinct tactics (mean=5.7, max=11), indicating that adversarial AI attacks are not single-step exploits but sustained, multi-phase campaigns requiring attacker investment across reconnaissance, staging, access, and impact. Attack-chain transition analysis confirms a canonical progression: Reconnaissance → Resource Development → Initial Access → Execution, with the most common single transition being Initial Access → Execution (16 cases), followed by Resource Development → Initial Access (14) and Resource Development → AI Attack Staging (12).

Critically, this attack complexity has governance implications. We compute mitigation demand by mapping each technique observed in the case corpus to the ATLAS mitigations that counter it. The five highest-demand mitigations — AI Telemetry Logging (demand=64, covering 12 techniques), Ensemble Methods (45), Access Control at Rest (43), Adversarial Input Detection (38), and I/O Validation for AI Agents (37) — share a common requirement: they depend on organizational infrastructure (logging pipelines, model access, input monitoring) that is precisely what the EO governance inventory shows is rarely provisioned. Table 6 maps the top ATLAS mitigations to their closest EO 13960 analogs.

**Table 6.** ATLAS mitigation demand → EO 13960 governance analog bridge

| ATLAS Mitigation | Demand | EO Analog | EO Prevalence |
|---|---:|---|---:|
| AI Telemetry Logging (M0024) | 64 | Post-deployment monitoring | 8.5% |
| Adversarial Input Detection (M0015) | 38 | Real-world testing | 8.5% |
| Model Hardening (M0003) | 35 | Impact assessment + Independent eval | 5.2–6.8% |
| I/O Validation for Agents (M0033) | 37 | No direct EO analog | — |
| Access Control at Rest (M0005) | 43 | Code access + Data catalog | 34–83% |

*Notes.* Demand = number of technique-mentions across 52 case studies that the mitigation counters. EO prevalence from Table 1 (n=1,757). Access Control shows wide range because code access (83%) is common while data catalog use varies substantially by agency.

The bridge table reveals a structural misalignment: the adversarial mitigations with the greatest demand — telemetry logging, adversarial input detection, model hardening — require exactly the transparency and monitoring infrastructure that §4.1 shows is implemented by fewer than 9% of federal AI systems. The highest-demand mitigation (AI Telemetry Logging, demand=64) maps to post-deployment monitoring, which only 8.5% of EO systems report. Meanwhile, the only ATLAS mitigation with substantial EO coverage is access control (code access at 83%), but this is precisely the control most degraded by vendor procurement (§4.6, OR=0.56). The emerging threat of agentic AI (I/O Validation for Agents, demand=37) has no EO analog at all, representing a governance blind spot. This Threat → Gap alignment reinforces the governance theater interpretation: organizations face multi-stage adversarial threats that require deep operational infrastructure, yet the governance inventory shows that such infrastructure is the exception rather than the norm.

### 4.4 Threat Landscape: Sector-Specific Incident Fingerprints
...
Two complementary AIID taxonomies illuminate sector-specific patterns at different levels of abstraction. The CSET taxonomy assigns *harm categories* (economic, physical, civil-rights, social) and sector labels; the GMF taxonomy assigns *technical failure types* (fairness, safety, performance, robustness). We use the term *sector–harm fingerprint* when discussing CSET harm associations and *sector–failure fingerprint* when discussing GMF technical failure associations; where the two converge we refer to *sector-specific incident fingerprints*.
...
**Coverage and robustness.** The sector–failure heatmap (Figure 3) draws on the intersection of CSET sector labels and GMF technical failure labels (n=135 incidents, 10% of the full AIID corpus). Table 5 shows the AIID coverage cascade.

**Table 5.** AIID coverage cascade

| Layer | n | % of total |
|---|---:|---:|
| Total AIID incidents | 1,362 | 100% |
| With text description | 1,362 | 100% |
| With CSET taxonomy (sector, harm) | 201 | 15% |
| With GMF taxonomy (tech failure) | 278 | 20% |
| With either taxonomy | 344 | 25% |
| With both sector + tech failure (heatmap input) | 135 | 10% |

To assess whether sector–harm associations generalize beyond the classified subset, we re-run the analysis using keyword-based text mining over all 1,362 incident descriptions, assigning sector (9 categories) and harm type (6 categories) via dictionary matching. The text-mined analysis yields n=398 incidents with both assignable sector and harm labels (2.9× the structured subset). The sector–harm association replicates with stronger statistical support (χ²=323.0, df=35, p<0.0001, Cramér’s V=0.403 vs. V=0.372 in the structured subset). Key fingerprints are preserved: finance → economic harm (68% of finance incidents), social media → misinformation (73%), and transportation → physical harm (75%). Sector–label concordance between structured and text-mined assignments is 55% (64/116 incidents with both), reflecting the expected noise from keyword approximation but confirming directional consistency.
**Sector–failure fingerprints and cross-level linkage.** Within these harm fingerprints, more granular modalities matter. Biometrics incidents show a disproportionate association with civil rights harm (EXP_168, p<0.01). Bridging the two taxonomy layers, fairness *failures* (GMF) map more strongly to intangible *harms* (CSET), while safety *failures* map to tangible *harms*; this failure-to-harm relationship is one of the most statistically robust results in the AIID analyses (EXP_173: χ²=12.97, p=0.0003). These cross-level patterns suggest that "trust readiness" should be understood as a configurable capability bundle whose emphasis (e.g., fairness evaluation vs. safety assurance vs. privacy controls) must be calibrated to sector and modality rather than uniformly applied.
...
### 4.5 Cross-Source Triangulation: Where Threats, Incidents, and Governance Diverge
...
### 4.6 Structural Barriers: Commercial Opacity and Procurement Governance
...
### 4.7 Governance Architecture: Bundling Effects and the Assessment-Action Gap
...
### 4.8 Pathway Test: Architecture Drives Deployment, Governance Does Not (Yet)
...
Regulatory pressure is theoretically central to our model: coercive isomorphic pressure should elevate trust readiness from operational compliance to strategic priority, making governance capability-building a license-to-operate condition [@dimaggio1983]. Our original step-4 proposition set (P5 in the pre-reconciliation package) predicted that external regulatory pressure increases the strategic salience of trust readiness, elevating it onto the CIO agenda. The EO 13960 data provide a natural operationalization: the inventory is itself a mandate product [@eo13960], every use case is subject to federal governance requirements, and 227 use cases carry an explicit rights-or-safety-impacting classification that triggers additional obligations. This creates a within-mandate gradient of regulatory pressure — from baseline federal requirements to heightened rights/safety mandates — that we can test directly.
...
### Governance theater as institutional decoupling, not mere immaturity

A central interpretive contribution of the findings is the characterization of "governance theater." We define governance theater as *the systematic decoupling of formal AI governance structures from the operational routines needed to enact them* — an AI-specific instantiation of the decoupling dynamic first identified by @meyer1977 and refined by @bromley2012. Our results allow a more precise diagnosis than the loose label of "superficial compliance." Governance theater is the co-occurrence of (a) moderate-to-high prevalence of surface compliance artifacts (internal review at 61%, ATO at 37%) and (b) very low prevalence of safeguards that would make authorization meaningful under real threat and harm conditions (impact assessment, bias mitigation, independent evaluation, monitoring).

Three empirical signatures differentiate theater from mere immaturity, and each maps onto a recognized form of institutional decoupling. First is the flatness of risk-tiering: high-impact and rights-impacting systems do not consistently report higher safeguard implementation. In @bromley2012’s terms, this is *policy-practice decoupling* — the adopted classification policy does not translate into differentiated operational practice. This matters because risk-tiering is a foundational assumption of contemporary governance designs (e.g., EU-style risk classification, RMF-based prioritization). Its failure suggests that governance tiering is not embedded as an operational routine. Second is the persistence of the gap despite mandates: timing analyses show no detectable improvement in disparity/bias mitigation after EO-era policy expectations, implying that formal requirements are insufficient without implementable capability infrastructure. Third is the assessment-action gap: impact assessments do not reliably translate into mitigation. This is *means-ends decoupling* — the practice is implemented but does not achieve its intended outcome [@bromley2012]. Assessments function as documentation artifacts rather than triggers for operational reconfiguration, consistent with the pattern @ali2023 document in AI ethics initiatives where formal structures absorb institutional pressure without altering engineering practice.

**Why governance theater occurs: a three-mechanism account.** Our findings point to three reinforcing mechanisms that sustain decoupling in AI governance. *(i) Procurement opacity.* Vendor-supplied systems block access to the artifacts — model weights, training data lineage, code — needed to execute substantive safeguards (§4.6). When the boundary of the system is controlled by an external party, even motivated governance actors cannot perform independent evaluation or bias mitigation, producing structural policy-practice decoupling. *(ii) Missing evaluation infrastructure.* The ATLAS threat characterization (§4.3) shows that the highest-demand adversarial mitigations — telemetry logging, adversarial input detection, model hardening — require monitoring pipelines, testing harnesses, and model-access infrastructure that only 5–9% of federal systems report. Without this infrastructure, governance policies cannot be operationalized, and assessments cannot cascade into mitigation. *(iii) Capability bundling thresholds.* Safeguards co-occur in clusters (§4.7), suggesting a non-linear capability-building dynamic: organizations must invest in coordinated bundles of sensing (assessment), seizing (testing/evaluation gates), and reconfiguring (monitoring/review) routines simultaneously. Partial investment — an assessment process without a testing harness, or a review board without data access — produces means-ends decoupling because the isolated control lacks the complementary routines needed to function.

From a DMC perspective, governance theater therefore indicates not just motivational deficit but microfoundation deficit: organizations may "want" trustworthy AI, but they lack the measurement infrastructure, transparency artifacts, and workflow gates needed to enact trustworthiness at scale. This interpretation also reframes the high contradiction rate: hypotheses encoded governance ideals (risk-tiering, lifecycle governance, autonomy-as-risk), and the consistent falsification of those ideals is evidence that capability assumptions do not hold in practice. Our contribution is to show that AI governance theater is not an amorphous organizational failing but a specific, theoretically grounded decoupling pattern with identifiable structural antecedents.

### Regulatory pressure without implementability: mandate as necessary but insufficient

The governance theater finding has a direct implication for the role of regulatory pressure in the conceptual model. Institutional theory predicts that coercive isomorphic pressure — binding mandates, enforcement regimes, classification requirements — should elevate trust readiness onto the strategic agenda and produce measurable governance investment [@dimaggio1983]. The EO 13960 context provides a strong test: every use case in the inventory exists *because of* a federal mandate, and the rights-or-safety-impacting subset (n=227) faces explicit additional obligations. Yet even under these heightened-pressure conditions, deep safeguards remain low: impact assessment at 22.5%, independent evaluation at 18.1%, bias mitigation at 16.3%. Three timing experiments (EXP_182, EXP_241, EXP_268) show no detectable improvement in disparity mitigation after EO-era policy expectations. Mandate strength alone does not close the governance gap.

This pattern recasts regulatory pressure as a *necessary but insufficient* condition for trust readiness. Mandates create demand for governance capability, but they do not supply the infrastructure — evaluation harnesses, data catalogs, telemetry pipelines, procurement transparency — that makes governance executable. In institutional-theory terms, coercive isomorphism produces the *form* of governance (review processes, classification schemes) without the *substance* (assessment-to-mitigation workflows, independent evaluation capacity) — precisely the policy-practice and means-ends decoupling mechanisms identified above. This is why we embed regulatory pressure as a boundary condition in P3 rather than as a standalone proposition: pressure amplifies the visibility of governance theater but does not resolve it. A testable extension is whether jurisdictions with enforcement mechanisms (e.g., the EU AI Act’s penalty provisions and conformity assessment requirements) produce higher substantive safeguard rates than mandate-only regimes such as the U.S. executive order framework — a cross-national comparison that would directly test whether enforcement, not merely mandate, is the lever that converts pressure into capability.
...
Third, governance should be sector-calibrated rather than autonomy-calibrated. The evidence indicates that sector and modality better predict harm profiles than autonomy level. CIOs should therefore align evaluation priorities and safeguards to sector-specific incident fingerprints — both the harm dimension (which harms predominate in a given sector) and the failure dimension (which technical failure modes are most frequent) — (e.g., civil rights and contestability controls in public-facing biometric systems; safety assurance in healthcare and physical-world AI; fairness and discrimination controls in finance).
